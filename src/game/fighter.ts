...
export class Fighter {
  ...
  constructor(playerNumber: 1 | 2, character: CharacterDef, startX: number, startY: number, isAi: boolean = false) {
    ...
    this.isAi = isAi;
    this.debugTokens = 1; // Start with 1 Debug Token ready for Ultimate Move
  }
  ...
  public resetForRound(startX: number, startY: number) {
    ...
    this.hp = 100;
    this.stamina = 100;
    this.debugTokens = Math.max(1, this.debugTokens);
    this.state = 'idle';
    this.stateTimer = 0;
  ...
  public takeDamage(hit: HitBox, attacker: Fighter): { blocked: boolean; damageDealt: number } {
    ...
    if (this.isBlocking && this.facing !== attackDir && !this.isLagged) {
      ...
      // Update attacker's combo count and award Debug Token on 3-hit streak
      attacker.comboCount += 1;
      attacker.comboTimer = 90; // 1.5s combo chain window
      if (attacker.comboCount >= 3 && attacker.debugTokens < 2) {
        attacker.debugTokens = Math.min(2, attacker.debugTokens + 1);
      }
    }
