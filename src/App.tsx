  ...
  const checkCollisions = useCallback(() => {
    ...
    if (p1.activeHitbox && !p1.hasHitCurrentAttack && p2.state !== 'ko') {
      ...
      if (
        ...
        if (!result.blocked && p1.comboCount >= 2) {
          floatingTextsRef.current.push({
            id: nextTextId.current++,
            text: `${p1.comboCount}x COMBO!`,
            x: p2.x,
            y: p2.y - 85,
            vy: -1.0,
            color: '#fbbf24',
    ...
    if (p2.activeHitbox && !p2.hasHitCurrentAttack && p1.state !== 'ko') {
      ...
      if (
        ...
        if (!result.blocked && p2.comboCount >= 2) {
          floatingTextsRef.current.push({
            id: nextTextId.current++,
            text: `${p2.comboCount}x COMBO!`,
            x: p1.x,
            y: p1.y - 85,
            vy: -1.0,
            color: '#fbbf24',
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (gameMode === 'versus_2p') {
        ...
        // Pukulan Ringan (Light Attack): Numpad 1, Slash (/), KeyI
        else if (
          e.code === 'Numpad1' ||
          e.code === 'Slash' ||
          e.code === 'KeyI'
        ...
        // Pukulan Berat (Heavy Attack): Numpad 5, Period (.), Quote ('), KeyO
        else if (
          e.code === 'Numpad5' ||
          e.code === 'Period' ||
          e.code === 'Quote' ||
    ...
    <div className="relative w-full h-screen bg-slate-950 text-slate-100 overflow-hidden font-sans select-none flex flex-col">
      ...
      {screen === 'match_end' && (
        <MatchEndModal
          ...
          onRematch={() => {
            setupMatch(p1CharId, p2CharId, arenaId, difficulty, gameMode);
            setScreen('fight');
          }}
      ...
      {/* 9. Pause Modal during Fight */}
      {screen === 'fight' && isPaused && (
        <PauseModal
